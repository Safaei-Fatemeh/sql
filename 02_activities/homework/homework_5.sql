...
SELECT  v.vendor_name, p.product_name,  SUM(5 * vi.original_price) AS total_sales
FROM vendor_inventory vi
JOIN vendor v ON vi.vendor_id = v.vendor_id
JOIN product p ON vi.product_id = p.product_id
CROSS JOIN customer c              -- CROSS JOIN with all customers
...
-- the last quantity for each product
